...
import org.openvision.visiondroid.helpers.enigma2.requesthandler.DeviceInfoRequestHandler;
...
public class CheckProfile {
	...
	public static final int[] FEATURE_EPGNOWNEXT_VERSION = { 1, 4, 1 };
	public static final int[] FEATURE_POST_REQEUEST = { 1, 4, 1 };
	...
	public static final int[] REQUIRED_VERSION = { 1, 4, 1 };
	@NonNull
	...
	public static ExtendedHashMap checkProfile(@NonNull Profile profile, Context context) {
		...
		VisionDroid.disableSleepTimer();
		VisionDroid.disableNowNext();
				...
				if (port > 0 && port <= 65535) {
					...
					if (xml != null && !shc.hasError()) {
						...
						if (dirh.parse(xml, deviceInfo)) {
							...
							if (vc >= 0) {
								int[] requiredForSleeptimer = { 1, 4, 1 };
								if (checkVersion(version, requiredForSleeptimer) >= 0)
									VisionDroid.enableSleepTimer();
								if (checkVersion(version, FEATURE_EPGNOWNEXT_VERSION) >= 0)
									VisionDroid.enableNowNext();
								if (checkVersion(version, FEATURE_POST_REQEUEST) >= 0)
									VisionDroid.setFeaturePostRequest(true);
								else
									VisionDroid.setFeaturePostRequest(false);
