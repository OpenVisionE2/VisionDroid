...
public class VirtualRemotePagerFragment extends BaseHttpFragment {
	...
	private class RemotePagerAdapter extends FragmentStatePagerAdapter {
		...
		public RemotePagerAdapter(@NonNull FragmentManager fm) {
			...
			args.putBoolean(VisionDroid.PREFS_KEY_QUICKZAP, false);
			f.setArguments(args);
			mItems.put(getString(R.string.quickzap), f);

			f = new VirtualRemoteFragment();
			args = new Bundle();
			args.putBoolean(VisionDroid.PREFS_KEY_QUICKZAP, true);
			f.setArguments(args);
	...
	public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (!sp.getBoolean(VisionDroid.PREFS_KEY_SIMPLE_VRM, true))
			mPager.setCurrentItem(1);
