...
import org.openvision.visiondroid.adapter.recyclerview.SimpleExtendedHashMapAdapter;
...
import org.openvision.visiondroid.helpers.SyncService;
...
public class ServiceListFragment extends BaseHttpRecyclerEventFragment {
	...
	public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		((AutofitRecyclerView) mDetailList).setMaxSpanCount(
				Integer.parseInt(
						prefs.getString(
								VisionDroid.PREFS_KEY_GRID_MAX_COLS,
	...
	public void showPopupMenu(@NonNull View v) {
		...
		menu.getMenu().findItem(R.id.menu_next_event).setVisible(VisionDroid.featureNowNext());
	...
	public void reload(@NonNull String ref, boolean isBouquetList) {
		...
		if (isBouquetList) {
			...
			if (VisionDroid.checkInitial(getAppCompatActivity(), VisionDroid.INITIAL_SERVICELIST_PANE)) {
				mSlidingPane.openPane();
				VisionDroid.setNotInitial(getAppCompatActivity(), VisionDroid.INITIAL_SERVICELIST_PANE);
			} else {
				mSlidingPane.closePane();
			}
			String param = "bRef";
