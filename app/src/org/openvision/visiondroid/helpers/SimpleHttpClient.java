...
import org.openvision.visiondroid.util.Base64;
...
public class SimpleHttpClient {
	...
	@NonNull
	public static String BIG_BUCK_BUNNY_URL = "https://visiondroid.org/bunny/big_buck_bunny_720p_h264.mov";
	...
	public String buildEncoderStreamUrl(String ref) {
		if (mProfile.getHost().equals("visiondroid.org"))
	...
	public boolean fetchPageContent(@NonNull String uri, @NonNull List<NameValuePair> parameters) {
		...
		try {
			...
			if (VisionDroid.featurePostRequest())
			...
			if (conn.getResponseCode() != 200) {
				if (conn.getResponseCode() == HttpURLConnection.HTTP_BAD_METHOD && mRememberedReturnCode != HttpURLConnection.HTTP_BAD_METHOD) {
					...
					VisionDroid.setFeaturePostRequest(!VisionDroid.featurePostRequest());
					conn.disconnect();
	...
	public void applyConfig() {
		if (mProfile == null)
			mProfile = VisionDroid.getCurrentProfile();
